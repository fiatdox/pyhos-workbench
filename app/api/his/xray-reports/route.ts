...
import { getXrayReports } from '@/lib/his/xray-reports'
...
export async function GET(request: Request) {
  ...
  const limited = rateLimit(`his-xray:${claims.sub}:${ip}`, PER_IP.limit, PER_IP.windowSeconds)
  if (!limited.ok) {
  ...
  try {
    const reports = await getXrayReports(hn)
    return Response.json({ success: true, hn, reports })
  } catch (error) {
    console.error('[his/xray-reports] ล้มเหลว:', error)
    return Response.json(
      { success: false, message: 'ดึงผลอ่านภาพรังสีไม่สำเร็จ กรุณาลองใหม่อีกครั้ง' },
      { status: 500 },
    )
  }
}

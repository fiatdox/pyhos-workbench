...
import { getPatientPhysicalExams } from '@/lib/his/physical-exams'
...
export async function GET(request: Request) {
  ...
  const limited = rateLimit(`his-pe:${claims.sub}:${ip}`, PER_IP.limit, PER_IP.windowSeconds)
  if (!limited.ok) {
  ...
  try {
    const exams = await getPatientPhysicalExams(hn)
    return Response.json({ success: true, hn, exams })
  } catch (error) {
    console.error('[his/physical-exams] ล้มเหลว:', error)

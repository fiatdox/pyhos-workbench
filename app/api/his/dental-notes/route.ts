...
import { getDentalNotes } from '@/lib/his/dental-notes'
...
export async function GET(request: Request) {
  ...
  const limited = rateLimit(`his-dental:${claims.sub}:${ip}`, PER_IP.limit, PER_IP.windowSeconds)
  if (!limited.ok) {
  ...
  try {
    const notes = await getDentalNotes(hn)
    return Response.json({ success: true, hn, notes })
  } catch (error) {
    console.error('[his/dental-notes] ล้มเหลว:', error)
    return Response.json(
      { success: false, message: 'ดึงบันทึกงานทันตกรรมไม่สำเร็จ กรุณาลองใหม่อีกครั้ง' },
      { status: 500 },
    )
  }
}
